        ...
        do {    // 1번의 계산이 끝난 후 exit 입력여부로 반복계산 실행
            ...
            if (select.equals("사칙연산")) {
                try {
                    ...
                    System.out.println("연산 결과 : " + cal.calculate(firstNumber, operator, secondNumber));
                } catch (BadInputException e) {    // Calculator 객체로 넘어간 데이터 상에서 예외사항 발생 시 핸들링
                ...
                if(sc.next().equals("modify")) {
                    System.out.println(cal.getResultArray().toString());
                    ...
                    cal.setResultArray(index, value);    // Calculator 사칙연산에 대한 setter 메서드 활용
                ...
                if (sc.next().equals("remove")) {
                    cal.removeResultArray();    // Calculator 객체의 removeResult 메서드를 통해 간접 접근해서 삭제로 수정
